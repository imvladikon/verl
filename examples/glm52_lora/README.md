...
The checked-in reconstruction tools never load the full donor or a complete
donor shard. `plan_glm52_surgery_pair.py` reads safetensors headers and small
router tensors through validated HTTP ranges; `build_glm52_surgery_dummy.py`
materializes resumable bounded-size shards and records a digest receipt for
every source range; `verify_glm52_surgery_pair.py` binds the finished BF16/FP8
pair to its immutable source revisions, tensor locations, expert selection,
and exact 128×128 FP8 scale grids. `run_glm52_fp8_twin_remote.sh` defaults to a
metadata-only dry run and requires `EXECUTE=1` before transferring weights.
...
### Current census-v11 candidate

The current full-model candidate is the independently censused
`mixture_targeted_wikipedia_v11_train_576` view. Stage the immutable artifacts described in
[`quality/README.md`](quality/README.md), then run MLA-only first:
...
The launcher verifies every view receipt, retokenizes the train rows with the
exact model tokenizer, consumes exactly 576 rows as nine global batches of 64,
validates on 160 rows, and never passes the 124-row test split to VERL. Its
576-token cap is above the measured 548-token maximum. The companion
`run_full_sft_census_v11_mla_lm_head_megatron.sh` changes only the adapter
surface. Both remain full-model runtime candidates; the 9B surgery fixture
proves the systems path but cannot prove language quality.

The held-out supplement is generated by
`build_heldout_quality_challenge.py`. It contains 70 validation and 70 test
examples spanning Russian quality, four Markdown families, spontaneous Han,
Han cleanup, and legitimate Chinese retention. Selection uses validation only;
test is opened once after the adapter choice.

### Historical v2 mixture — do not launch

The commands and measurements below are retained only to explain earlier
systems evidence. Cross-split target reuse invalidated this data for training
selection and quality evaluation. Both old locked-mixture launchers now exit
immediately with `HISTORICAL-INVALID-DATA`.
