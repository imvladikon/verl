...
### Current clean-v4 candidate

The current full-model candidate is the split-isolated
`mixture_targeted_wikipedia_v4_train_1792` view, not the older 2,728-row
mixture below. Stage the immutable artifacts described in
...
The launcher verifies every source/view receipt, consumes exactly 1,792 train
rows as 28 global batches of 64, validates the unchanged 244-row validation
split, and never passes the 184-row test split to VERL. The companion
`run_full_sft_clean_v4_mla_lm_head_megatron.sh` changes only the adapter
surface. Both are full-model runtime candidates; the 9B surgery fixture proves
the systems path but cannot prove language quality.
