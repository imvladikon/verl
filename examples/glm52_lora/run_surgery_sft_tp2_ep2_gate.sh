...
# Four-GPU combined TP2+EP2 adapter save/resume/export/reload gate for the
# immutable GLM-5.2 surgery checkpoint. This exercises the process-group
# interaction that separate TP2 and EP2 gates cannot cover.
...
run_dir=${RUN_DIR:-${repo_root}/runs/glm52-lora-surgery-sft-tp2-ep2-gate}
gpu_list=${GPU_IDS:-}
rank=${LORA_RANK:-16}
alpha=${LORA_ALPHA:-32}
steps=${STEPS:-2}
...
if (( rank != 16 || alpha != 32 || steps != 2 )); then
  echo "TP2+EP2 gate is locked to rank 16 / alpha 32 / two steps" >&2
  exit 2
fi
...
if [[ "${config_only}" != 1 ]]; then
  if [[ "${TP2_EP2_GATE_ACK:-}" != GLM52_TP2_EP2_MLA_R16 ]]; then
    echo "set TP2_EP2_GATE_ACK=GLM52_TP2_EP2_MLA_R16 after auditing the allocation" >&2
    exit 4
  fi
  IFS=',' read -r -a gpu_ids <<<"${gpu_list}"
  if (( ${#gpu_ids[@]} != 4 )); then
    echo "GPU_IDS must contain exactly four distinct physical GPU indices" >&2
    exit 4
  fi
  ...
  for gpu_id in "${gpu_ids[@]}"; do
    ...
    if [[ -n "${seen_gpu_ids[${gpu_id}]:-}" ]]; then
      echo "duplicate physical GPU index: ${gpu_id}" >&2
      exit 4
    fi
...
if [[ "${config_only}" != 1 && -e "${run_dir}" ]]; then
  echo "refusing existing TP2+EP2 run directory: ${run_dir}" >&2
  exit 4
fi
...
job_args=(
  ...
  trainer.experiment_name=surgery-sft-tp2-ep2-gate
...
"${python_bin}" "${script_dir}/verify_tp2_ep2_sft_config.py" \
...
if [[ ! -d "${checkpoint_step2}" ]]; then
  echo "missing initial TP2+EP2 checkpoint: ${checkpoint_step2}" >&2
  exit 5
fi
...
resume_args=(
  ...
  trainer.experiment_name=surgery-sft-tp2-ep2-resume-gate
...
if [[ ! -d "${checkpoint_step3}" ]]; then
  echo "missing resumed TP2+EP2 checkpoint: ${checkpoint_step3}" >&2
  exit 5
fi
"${python_bin}" "${script_dir}/verify_surgery_adapter.py" \
  "${checkpoint_step3}" | tee "${run_dir}/adapter_verification.json"
"${python_bin}" "${script_dir}/verify_tp2_ep2_resume_run.py" \
...
(
  ...
  sha256sum \
    ...
    >tp2_ep2_gate_inputs.sha256
  sha256sum tp2_ep2_gate_inputs.sha256 | cut -d' ' -f1 >tp2_ep2_gate.sha256
)

printf 'TP2_EP2_GATE_PASS sha256=%s\n' "$(<"${run_dir}/tp2_ep2_gate.sha256")"
