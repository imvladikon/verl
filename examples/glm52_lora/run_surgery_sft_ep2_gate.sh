...
# Two-GPU EP2 routed-expert and adapter save/resume gate for the GLM-5.2 surgery model.
...
run_dir=${RUN_DIR:-${repo_root}/runs/glm52-lora-surgery-sft-ep2-gate}
gpu_list=${GPU_IDS:-}
rank=${LORA_RANK:-16}
alpha=${LORA_ALPHA:-32}
steps=${STEPS:-2}
...
if (( rank != 16 || alpha != 32 || steps != 2 )); then
  echo "EP2 gate is locked to rank 16 / alpha 32 / two steps" >&2
  exit 2
fi
...
if [[ "${config_only}" != 1 ]]; then
  if [[ "${EP_GATE_ACK:-}" != GLM52_EP2_MLA_R16 ]]; then
    echo "set EP_GATE_ACK=GLM52_EP2_MLA_R16 after auditing the allocation" >&2
    exit 4
  fi
...
if [[ "${config_only}" != 1 && -e "${run_dir}" ]]; then
  echo "refusing existing EP2 run directory: ${run_dir}" >&2
  exit 4
fi
...
job_args=(
  ...
  trainer.experiment_name=surgery-sft-ep2-gate
...
"${python_bin}" "${script_dir}/verify_ep2_sft_config.py" \
...
if [[ ! -d "${checkpoint_step2}" ]]; then
  echo "missing initial EP2 checkpoint: ${checkpoint_step2}" >&2
  exit 5
fi
...
resume_args=(
  ...
  trainer.experiment_name=surgery-sft-ep2-resume-gate
...
if [[ ! -d "${checkpoint_step3}" ]]; then
  echo "missing resumed EP2 checkpoint: ${checkpoint_step3}" >&2
  exit 5
fi
"${python_bin}" "${script_dir}/verify_surgery_adapter.py" \
  "${checkpoint_step3}" | tee "${run_dir}/adapter_verification.json"
"${python_bin}" "${script_dir}/verify_ep2_resume_run.py" \
...
(
  ...
  sha256sum \
    ...
    >ep2_gate_inputs.sha256
  sha256sum ep2_gate_inputs.sha256 | cut -d' ' -f1 >ep2_gate.sha256
)

printf 'EP2_GATE_PASS sha256=%s\n' "$(<"${run_dir}/ep2_gate.sha256")"
