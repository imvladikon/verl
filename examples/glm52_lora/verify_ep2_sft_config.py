#!/usr/bin/env python3
"""Validate the resolved two-GPU GLM-5.2 surgery LoRA SFT gate."""
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise AssertionError(f"CONFIG-FAIL: {message}")
...
def verify_config(
    ...
    *,
    ...
    require(engine["tensor_model_parallel_size"] == 1, "TP drift")
    ...
    require(engine["sequence_parallel"] is False, "sequence parallel unexpectedly enabled")
    ...
    require(trainer["n_gpus_per_node"] == 2, "GPU-count drift")
    ...
    return {
        ...
        "required_runtime_evidence": [
            "two finite optimizer steps",
            "EP2 routed-expert forward/backward and distributed adapter checkpoint",
            "distributed model/optimizer/extra/dataloader resume to the next step",
            "HF adapter export and finite nonzero LoRA-B tensors",
            "fresh single-GPU adapter reload with nonzero finite logit delta",
        ],
    }
