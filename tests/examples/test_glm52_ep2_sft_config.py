...
from verify_ep2_resume_run import verify_run  # noqa: E402
from verify_ep2_sft_config import EXPECTED_TARGETS, verify_config  # noqa: E402
...
def test_ep2_verifier_accepts_exact_contract(tmp_path: Path) -> None:
    ...
    assert result["topology"] == {"nodes": 1, "gpus": 2, "tp": 1, "ep": 2, "dp": 2, "expert_dp": 1}


def test_ep2_verifier_accepts_hydra_single_path_form(tmp_path: Path) -> None:
    config, model, train, run = valid_config(tmp_path)
...
def test_ep2_verifier_accepts_exact_resume_contract(tmp_path: Path) -> None:
    config, model, train, run = valid_config(tmp_path)
...
@pytest.mark.parametrize(
    ("path", "value", "message"),
    [
        (("engine", "tensor_model_parallel_size"), 2, "TP drift"),
        (("engine", "expert_model_parallel_size"), 1, "EP drift"),
        (("engine", "sequence_parallel"), True, "sequence parallel unexpectedly enabled"),
        (("data", "train_batch_size"), 1, "global batch drift"),
...
def test_ep2_verifier_rejects_contract_drift(
    tmp_path: Path,
    path: tuple[str, ...],
    value: object,
    message: str,
...
def test_ep2_verifier_rejects_resume_checkpoint_drift(tmp_path: Path) -> None:
    config, model, train, run = valid_config(tmp_path)
...
def test_ep2_runtime_verifier_accepts_complete_resume(tmp_path: Path) -> None:
...
def test_ep2_runtime_verifier_rejects_nan_gradient(tmp_path: Path) -> None:
...
def test_ep2_runtime_verifier_rejects_restarted_dataloader(tmp_path: Path) -> None:
    run = write_runtime_fixture(tmp_path)
    resume_log = run / "resume.log"
    resume_log.write_text(resume_log.read_text(encoding="utf-8").replace("global_tokens:214", "global_tokens:186"))
