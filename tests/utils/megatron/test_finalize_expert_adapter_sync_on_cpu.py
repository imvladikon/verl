...
def test_bridge_wrapper_is_installed_and_fallback_hooks_are_removed(monkeypatch):
    ...
    model = [SimpleNamespace(parameters=lambda: [SimpleNamespace(expert_parallel_replicated=True)])]
...
def test_megatron_finalize_without_bridge(monkeypatch, replicated):
    ...
    model = [SimpleNamespace(parameters=lambda: [SimpleNamespace(expert_parallel_replicated=replicated)])]
    if replicated:
        with pytest.raises(ImportError):
            megatron_utils._finalize_model_grads_func(model)
    else:
        assert megatron_utils._finalize_model_grads_func(model) is finalize_model_grads
