...
def test_plan_binds_hf_and_serving_geometry_without_indexer_or_embeddings():
    ...
    assert sglang_lora_target_modules(plan["target_modules"], plan) == plan["rollout_target_modules"]

    peft_config = LoraConfig(
        target_modules=plan["target_modules"],
        exclude_modules=plan["trainer_exclude_modules"],
    )
    assert not check_target_module_exists(peft_config, "model.visual.merger.gate_proj")
    assert check_target_module_exists(peft_config, "model.language_model.layers.0.mlp.gate_proj")
...
def test_plan_preserves_user_exclusions_and_fingerprints_them():
    ...
    custom = build_glm5_next_lora_adapter_plan(config, "all-linear", exclude_modules=["layers.0.self_attn.q_proj"])
    peft_config = LoraConfig(
    ...
    assert not check_target_module_exists(peft_config, "model.language_model.layers.0.self_attn.q_proj")
    assert not check_target_module_exists(peft_config, "base_model.model.model.visual.merger.down_proj")
    assert check_target_module_exists(peft_config, "model.language_model.layers.0.self_attn.k_proj")
...
def test_rollout_detects_post_plan_target_drift():
    plan = build_glm5_next_lora_adapter_plan(glm_config(["linear_attention"]), "all-linear")
...
def test_non_glm_and_explicit_targets_keep_existing_semantics():
    assert build_glm5_next_lora_adapter_plan(SimpleNamespace(model_type="qwen2"), "all-linear") is None
    assert build_glm5_next_lora_adapter_plan(glm_config(["linear_attention"]), ["q_proj"]) is None
    assert sglang_lora_target_modules("all-linear") == ["all"]


def test_9b_sft_recipe_is_portable_and_runs_two_steps_by_default():
    source = (REPO_ROOT / "examples/glm53_flash/run_glm53_flash_9b_lora_sft_smoke.sh").read_text()
...
def test_9b_rl_recipe_uses_adapter_sync_and_loads_the_base_checkpoint():
    source = (REPO_ROOT / "examples/glm53_flash/run_glm53_flash_9b_lora_rl_smoke.sh").read_text()
    assert "/home/" not in source
    assert "TRANSFORMERS_SOURCE" in source
    assert "TRANSFORMERS_COMMIT" in source
    assert "GLM53_OPTIMIZER_IMPL=torch.optim" in source
    assert "actor_rollout_ref.model.target_modules=all-linear" in source
    assert "actor_rollout_ref.rollout.load_format=safetensors" in source
    assert "+actor_rollout_ref.actor.checkpoint.save_lora_only=true" in source
